...
from config import (
    ...
    BBL_SUMMARY_DIR, BBL_LOGS_DIR, get_app_suite
)
...
def profile_single_app_with_retry(app_name, timeout, max_retries):
    ...
    Returns:
        dict: 剖析结果
    """
    from profile_single import BBLProfiler

    for attempt in range(max_retries + 1):
        try:
            profiler = BBLProfiler(app_name)
...
class BatchBBLProfiler:
    ...
    def __init__(self, apps, parallel=1, timeout=PROFILE_TIMEOUT,
        ...
        self.batch_log = os.path.join(BBL_LOGS_DIR, f"batch_bbl_profile_{timestamp}.log")
        self.summary_json = os.path.join(BBL_SUMMARY_DIR, f"batch_summary_{timestamp}.json")
