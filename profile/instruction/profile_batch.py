...
from config import (
    ...
    INSTRUCTION_SUMMARY_DIR, INSTRUCTION_LOGS_DIR, get_app_suite
)
...
def profile_single_app_with_retry(app_name, timeout, max_retries):
    """
    剖析单个应用(带重试，用于并行处理)
    ...
    Returns:
        dict: 剖析结果
    """
    from profile_single import InstructionProfiler

    for attempt in range(max_retries + 1):
        try:
            profiler = InstructionProfiler(app_name)
...
class BatchInstructionProfiler:
    """批量指令剖析器类"""
    ...
    def __init__(self, apps, parallel=1, timeout=PROFILE_TIMEOUT,
        ...
        self.batch_log = os.path.join(INSTRUCTION_LOGS_DIR, f"batch_instruction_profile_{timestamp}.log")
        self.summary_json = os.path.join(INSTRUCTION_SUMMARY_DIR, f"batch_summary_{timestamp}.json")
    ...
    def _run_sequential(self):
        ...
        for app_name in iterator:
            result = profile_single_app_with_retry(
                app_name, self.timeout, self.max_retries
            )
...
示例:
  ...
  python profile_batch.py --all --parallel 2
    ...
    # 创建批量剖析器并运行
    profiler = BatchInstructionProfiler(
        apps=apps,
