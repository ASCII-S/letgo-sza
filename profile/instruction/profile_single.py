...
2. 调用instruction_profiler进行指令级剖析
3. 保存JSON结果
4. 处理MPI应用的特殊情况
5. 错误处理和日志记录
"""
...
from config import (
    PIN_BINARY, INSTRUCTION_PROFILER_TOOL,
    get_app_config, get_instruction_output_json_path,
    PROFILE_TIMEOUT, MPI_APP, INSTRUCTION_LOGS_DIR
)
...
class InstructionProfiler:
    ...
    def __init__(self, app_name, output_json=None):
        ...
        self.output_json = output_json or get_instruction_output_json_path(app_name)
        ...
        self.log_file = os.path.join(INSTRUCTION_LOGS_DIR, f"{app_name}_instruction_profile_{timestamp}.log")
    ...
    def _build_command(self):
        ...
        # Pin 和工具
        cmd.extend([PIN_BINARY, '-t', INSTRUCTION_PROFILER_TOOL])
    ...
    def run(self, timeout=PROFILE_TIMEOUT):
        ...
        try:
            ...
            try:
                ...
                assert 'instructions' in json_data, "缺少instructions列表"
                assert isinstance(json_data['instructions'], list), "instructions必须是列表"

                file_size = os.path.getsize(self.output_json) / (1024 * 1024)  # MB
                num_instructions = len(json_data['instructions'])
...
示例:
  ...
  python profile_single.py hpl --timeout 7200
    ...
    parser.add_argument('--output', '-o', type=str, default=None,
                       help='输出JSON文件路径(默认：results/raw_json/<suite>/<app>_instruction_profile.json)')
