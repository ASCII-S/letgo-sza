...
CSV_FIELDS_PER_APP = [
    ...
    'total_crashes', 'overall_crash_rate', 'unique_disasm_count',
]
...
def simulate_threshold(reports: dict, threshold: float) -> dict:
    """
    模拟给定阈值下的注错结果。
    ...
    unique_disasms = set(t.get('disasm', '') for t in selected_targets)
    ...
    return {
        ...
        'unique_disasm_count': len(unique_disasms),
    }


def run_simulation(benchmark_dir: Path) -> list:
    """对所有阈值运行模拟，返回结果列表。"""
    ...
    for th in THRESHOLDS:
        result = simulate_threshold(reports, th)
        result['threshold'] = th
...
def main():
    ...
    for name, adaptive_dir in benchmarks:
        print(f"[{name}] 处理中...")
        results = run_simulation(adaptive_dir)
        if not results:
            print(f"  跳过: 无有效数据")
            continue
